...
public class Converter {
	...
	// new AnnotationEntryParser<EclipseDataEntry>(EclipseDataEntry.class,
	// provider);
	// CSVReader<EclipseDataEntry> csvPersonReader =
	// new
	// CSVReaderBuilder<EclipseDataEntry>(reader).entryParser(entryParser).build();
	//
	...
	private static List<GenericTrace> readGenericLog2(String filename) throws IOException {
		...
		CSVReader<GenericTrace> csvTraceReader = new CSVReaderBuilder<GenericTrace>(reader).strategy(strategy)
				.entryParser(genericTraceParser).build();
	...
	// private static XLog convertEclispeDataEntries(List<EclipseDataEntry>
	// entriesList){
	//
	// if(entriesList == null)
	// return null;
	...
	//
	// // When I find a new new UserId (representing the Case Id) I create a new
	// trace
	...
	// timeExtension.assignTimestamp(xEvent, new
	// Timestamp(eclipseDataEntry.getTimestamp()));
	// xTrace.add(xEvent);
	// }//end loop
	//
	// return xLog;
	...
	private static XLog toXlog2(List<GenericTrace> traces) {
		...
		for (GenericTrace trace : traces) {
			...
			lifeExtension.assignTransition(xEvent, trace.getLifecycleColumn());
			//lifeExtension.assignStandardTransition(xEvent, XLifecycleExtension.StandardModel.START);
